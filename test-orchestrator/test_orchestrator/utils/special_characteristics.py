...
def validate_notification_payload(header: Dict, content: Dict):
    """
    Validate the structure, required fields, and formatting of a Catena-X
    notification payload.

    Steps performed:
    1. Validate mandatory header fields such as UUIDs, timestamps, and BPNs.
    2. Validate `listOfEvents` structure and required event fields.
    3. Accumulate and report all validation errors.

    - :header (Dict): Header section of the notification payload.
    - :content (Dict): Content section of the notification payload.
...
async def validate_payload(receiver_bpn: str, events: List, max_events: int):
    """
    Validate event data and enforce maximum allowed event count.

    Steps performed:
    1. Validate that the number of events does not exceed the configured limit.

    - :receiver_bpn (str): BPN of the notification receiver.
    - :events (List): List of events from the notification payload.
...
async def process_notification_and_retrieve_dtr(
    ...
    receiver_bpn, events = await validate_payload(receiver_bpn, events, max_events)
